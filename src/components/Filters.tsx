...
export const Filters: React.FC = () => {
  ...
  const handleChangeCategory = (event: React.MouseEvent<HTMLButtonElement>) => {
    ...
    setFilters({
      category: category === 'fragancias' ? ['fragancia-masculina', 'fragancia-femenina'] : [category],
      searched: ''
    });
  }
    ...
    <div className="w-full overflow-x-auto py-2">
      <div className="inline-flex justify-start min-w-max px-4">
        <ul className="flex gap-6">
          <li><button onClick={handleChangeCategory} value='fragancias' className="border font-medium px-2 py-2 hover:rounded-md flex flex-col items-center transition-transform transform hover:border-gray-300" >Fragancias<img src={Fragancias} style={{ width: 90, height: 80 }} className='rounded-xl object-cover' /> </button></li>
